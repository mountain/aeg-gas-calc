# -*- coding: utf-8 -*-
"""
1D 稳态导热（GasSim）：沿 x 的温度剖面 + δ-理论（物理定标）+ 傅立叶对照 + 体/界/边闭合
---------------------------------------------------------------------------------
依赖：gassim>=0.0.6, numpy, matplotlib

要点：
- 采样几何由平均自由程 λ_mfp 决定；内域剖面避开靠壁薄层。
- 局域温度 T(x)：去对流、3D 单原子气体 T=(2/3)<(1/2)|c|^2>。
- δ–理论：先以 s(x)=∫dx/λ_mfp(x) 标定 (μ,λ)，令 y=asinh((λ/μ)β) 对 s 线性，再还原 Tδ(x)。
- 傅立叶：κ(T)=κ0 T^α → T^{α+1}(x) 线性（用两端气相侧温做内域边界条件）。
- 闭合检验：Σ_tot = Σ_bulk^fit + Σ_int^fit（逐窗）。
"""
...
N_WINDOWS     = 200
MIN_COUNT     = 5
MARGIN_LAMBDA = 0.75      # 剖面内域离壁 ≥ 0.75×λ_mfp

# 近壁“薄带”几何（气相侧 β(0+), β(L-)）
FACE_BAND_HALF_FACTOR = 0.5   # 半宽 = 0.5 × λ_mfp（不足自动放宽）
...
# 边界热量与热流（逐窗）
dQL_list, dQR_list = [], []
heat_data= sim.get_heat_by_wall()
Q_L_prev, Q_R_prev = heat_data[0], heat_data[1]
...
def accumulate_face_band(pos, vel, x_center, half):
    ...
    for f in (1.0, 1.5, 2.0):
        ...
        Ni = int(np.count_nonzero(mask))
        if Ni >= MIN_COUNT:
            T, E, Np = local_T_from_mask(vel[mask])
            return E, Np
    return 0.0, 0
...
errs = []
for a_ in alphas:
    errs.append(rmse_interp(x_grid_in, T_fourier_power(x_grid_in, T_L_gas, T_R_gas, x_L, x_R, a_)))
...
S = s_i[mask_int]
k,b = np.linalg.lstsq(np.vstack([S, np.ones_like(S)]).T, y_lin, rcond=None)[0]
plt.plot(S, y_lin, 'o', label='data y(s)')
plt.plot(S, k*S+b, '-', label=f'fit: slope≈{k:.4f} (λ̂), intercept≈{b:.4f}')
...
# 由拟合曲线取两端“气相侧”β^fit（此处直接用测得的 gas-side；也可换成 T_delta_collision(x_face_*)）
T_L_fit = float(T_delta_collision([x_face_Lc]))
T_R_fit = float(T_delta_collision([x_face_Rc]))
beta_L_fit, beta_R_fit = 1.0/T_L_fit, 1.0/T_R_fit

beta_L_bath, beta_R_bath = 1.0/TL, 1.0/TR
Sigma_tot  = A * Jq_arr * (beta_R_bath - beta_L_bath)      # 边界 δ（总熵率）
